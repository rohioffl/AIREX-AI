...
async def transition_state(
    ...
    from airex_core.core.metrics import state_transition_total
    ...
    try:
        from airex_core.services.notification_service import notify_incident_state_change
    ...
    if new_state in TERMINAL_STATES:
        ...
        try:
            from airex_core.services.resolution_service import record_resolution
        ...
        try:
            from airex_core.services.incident_embedding_service import (
        ...
        if new_state == IncidentState.RESOLVED:
            try:
                ...
                from airex_core.core.config import settings as _settings
                ...
                # Enqueue post-mortem generation
                await pool.enqueue_job(
                    "generate_postmortem_task",
                    str(incident.tenant_id),
                    str(incident.id),
                    _defer_by=10,  # delay after runbook generation
                )
                await pool.aclose()
            except (
                ConnectionError,
