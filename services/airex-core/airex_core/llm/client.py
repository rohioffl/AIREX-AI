...
from airex_core.core.metrics import ai_failure_total, ai_request_total, circuit_breaker_state
from airex_core.llm import build_llm_headers, configure_litellm
from airex_core.llm.prompts import build_chat_messages, build_recommendation_prompt
...
class LLMClient:
    ...
    async def generate_text(
        ...
        """Generate text from a prompt (for post-mortems, summaries, etc.).
        ...
        if not self.circuit_breaker.can_attempt():
            logger.warning("llm_text_generation_circuit_breaker_open")
            return None
        ...
        # Attempt primary model
        result = await self._call_text_model(
            settings.LLM_PRIMARY_MODEL,
            messages,
        ...
        # Attempt fallback model
        logger.info("llm_text_primary_failed", fallback=settings.LLM_FALLBACK_MODEL)
        result = await self._call_text_model(
            settings.LLM_FALLBACK_MODEL,
            messages,
            max_tokens=max_tokens,
    ...
    async def _call_text_model(
        ...
        """Call LLM for text generation (returns raw text, not JSON)."""
        try:
            import litellm
            ...
            logger.warning("llm_text_timeout", model=model, timeout=timeout)
            return None
        except Exception as exc:
            logger.warning("llm_text_failed", model=model, error=str(exc))
            return None
