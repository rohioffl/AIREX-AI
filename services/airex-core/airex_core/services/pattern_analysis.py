...
def _analyze_host_patterns(
    ...
    host_incidents = [i for i in historical if i.host_key == incident.host_key]
    ...
    same_alert_count = sum(
        1 for i in host_incidents if i.alert_type == incident.alert_type
    )
    ...
    resolved_count = sum(1 for i in host_incidents if i.state == IncidentState.RESOLVED)
    if resolved_count > 0:
...
def _analyze_alert_type_patterns(
    ...
    same_type = [i for i in historical if i.alert_type == incident.alert_type]
    ...
    resolved = [i for i in same_type if i.state == IncidentState.RESOLVED]
...
def _analyze_correlation_patterns(
    ...
    if incident.alert_type in alert_type_counts:
        ...
        other_types = [
            at
            for at, count in alert_type_counts.items()
...
def _build_pattern_context(
    ...
    if same_type:
        ...
        if recent_same_type:
            lines.append(
                f"Recent Similar Incidents (last 7 days): {len(recent_same_type)}"
            )
