...
class DrainNodeAction(BaseAction):
    ...
    DESCRIPTION = (
        "Drain a Kubernetes node by cordoning it and evicting all pods. Use when a node "
        "is unhealthy, has hardware issues, or needs maintenance. Blast radius: all pods "
        "on the node are rescheduled to other nodes. Risk: temporary capacity reduction; "
        "pods with no replicas will have downtime. Verification: node cordoned, pods "
        "rescheduled and healthy on other nodes."
    )
    ...
    async def _execute_aws(self, instance_id, commands, region, meta):
        try:
            ...
            success = (
                "cordoned" in output.lower()
                or "drained" in output.lower()
    ...
    async def verify(self, incident_meta: dict) -> bool:
        ...
        commands = [
            f"kubectl get node {node_name} 2>/dev/null | grep -q SchedulingDisabled && echo 'CORDONED' || echo 'NOT_CORDONED'"
        ]
