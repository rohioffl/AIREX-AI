...
from app.models.enums import IncidentState, RiskLevel, SeverityLevel
from app.services.fallback_service import attempt_fallback, select_next_alternative
...
class TestSelectNextAlternative:
    def test_returns_first_valid_alternative(self):
        ...
        with patch("app.services.fallback_service.ACTION_REGISTRY", {
    ...
    def test_respects_max_fallback_limit(self):
        ...
        with patch("app.services.fallback_service.settings") as mock_settings:
...
class TestAttemptFallback:
    ...
    async def test_initiates_fallback_with_human_approval(self):
        ...
        with patch("app.services.fallback_service.ACTION_REGISTRY", {
            "restart_service": MagicMock(),
            "scale_up_instances": MagicMock(),
        }), patch("app.services.fallback_service.transition_state", new_callable=AsyncMock) as mock_ts, \
             patch("app.services.fallback_service.check_policy", return_value=(True, "ok")), \
             patch("app.services.fallback_service.evaluate_approval") as mock_eval:
    ...
    async def test_auto_approves_high_confidence_fallback(self):
        ...
        with patch("app.services.fallback_service.ACTION_REGISTRY", {
            ...
            "scale_up_instances": MagicMock(),
             ...
             patch("app.services.fallback_service.check_policy", return_value=(True, "ok")), \
             patch("app.services.fallback_service.evaluate_approval") as mock_eval, \
             patch("app.services.fallback_service.settings") as mock_settings:
            mock_settings.MAX_FALLBACK_ALTERNATIVES = 2
    ...
    async def test_skips_policy_rejected_and_tries_next(self):
        ...
        }), patch("app.services.fallback_service.transition_state", new_callable=AsyncMock), \
             patch("app.services.fallback_service.check_policy", side_effect=mock_check_policy), \
             patch("app.services.fallback_service.evaluate_approval") as mock_eval:
            mock_eval.return_value = MagicMock(
                level=MagicMock(value="operator"),
