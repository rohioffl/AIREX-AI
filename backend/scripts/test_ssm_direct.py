...
sys.path.insert(0, str(__file__).replace("/scripts/test_ssm_direct.py", ""))

from app.cloud.aws_ssm import ssm_check_instance_managed, ssm_run_command
from app.cloud.discovery import discover_aws_instance
...
async def test_ssm_direct(instance_id: str = "", ip: str = "", region: str = ""):
    ...
    if ip and not instance_id:
        ...
        if not discovered:
            ...
            print("   1. Configure AWS credentials (see backend/config/tenants.yaml)")
            print("   2. Or provide instance ID directly: python -m scripts.test_ssm_direct i-xxxxx")
            return False
    ...
    if not is_managed:
        print(f"❌ Instance is NOT managed by SSM")
        print("\n📋 Requirements for SSM:")
        print("   1. SSM Agent must be installed and running")
        print("   2. Instance must have IAM role with 'AmazonSSMManagedInstanceCore' policy")
        ...
        print("   - Check IAM role: aws ec2 describe-instances --instance-ids {instance_id}")
        print("   - Check SSM status: aws ssm describe-instance-information --filters Key=InstanceIds,Values={instance_id}")
        return False
    
    print(f"✅ Instance IS managed by SSM")
    ...
    try:
        ...
        print(f"✅ Diagnostic command executed successfully!")
        print(f"\n📋 Diagnostic Output:")
        print("-" * 60)
        print(output)
        print("-" * 60)
...
def main():
    ...
    parser.add_argument("instance_id", nargs="?", help="EC2 instance ID (e.g. i-1234567890abcdef0)")
    ...
    success = asyncio.run(test_ssm_direct(
