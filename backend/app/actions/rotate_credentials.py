...
class RotateCredentialsAction(BaseAction):
    ...
    DESCRIPTION = (
        "Rotate expired SSL certificates or API keys and reload affected services. "
        "Use when SSL certificate expiry, compromised credentials, or authentication "
        "failures are detected. Blast radius: services using the rotated credential. "
        "Risk: brief auth failures during rotation window. Verification: SSL handshake "
        "succeeds, API authentication works."
    )
    ...
    async def _execute_aws(self, instance_id, commands, region, meta):
        try:
            ...
            success = (
                ...
                or "renewed" in output.lower()
                or "reload" in output.lower()
            )
            return ActionResult(
                success=success,
    ...
    async def verify(self, incident_meta: dict) -> bool:
        ...
        commands = [
            "openssl x509 -enddate -noout -in /etc/ssl/certs/server.crt 2>/dev/null || echo 'ok'"
        ]
