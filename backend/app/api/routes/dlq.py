...
from app.core.worker import DLQ_KEY
...
async def list_dlq(
    ...
    all_entries = await redis.lrange(DLQ_KEY, 0, -1)
    ...
    for entry_bytes in all_entries:
        try:
            entry_str = entry_bytes.decode() if isinstance(entry_bytes, bytes) else str(entry_bytes)
...
async def replay_dlq_entry(
    ...
    entry_bytes = await redis.lindex(DLQ_KEY, entry_index)
    ...
    # Remove from DLQ
    await redis.lrem(DLQ_KEY, 1, entry_bytes)
    ...
    try:
        ...
        await redis.rpush(DLQ_KEY, entry_bytes)
