...
class TestCheckSite24x7Monitors:
    ...
    async def test_processes_monitor_data(self):
        ...
        with (
            ...
            patch(
                "airex_core.monitoring.site24x7_client.Site24x7Client",
...
class TestAutoCreateIncidents:
    ...
    async def test_creates_incident_for_down_check(self):
        ...
        with (
            patch(
                "airex_core.services.incident_service.create_incident",
...
class TestRunHealthChecks:
    ...
    async def test_orchestrates_checks_and_incidents(self):
        ...
        with (
            patch("airex_core.core.database.get_tenant_session") as mock_session_ctx,
            patch(
                "airex_core.services.health_check_service.check_site24x7_monitors",
                new_callable=AsyncMock,
                return_value=checks,
            ),
            patch(
                "airex_core.services.health_check_service.auto_create_incidents",
                new_callable=AsyncMock,
                return_value=2,
            ),
        ):
