...
export default function MetricCard({ title, value, trend, trendType, isCritical, icon: Icon, colorVariant }) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -3, boxShadow: isCritical ? '0 14px 36px rgba(249,115,22,0.22)' : '0 8px 24px rgba(99,102,241,0.15)' }}
      whileTap={{ scale: 0.98 }}
    >
      <div className="flex items-center justify-between mb-4">
        ...
        {isCritical && (
          <span className="w-2.5 h-2.5 rounded-full animate-pulse" style={{ background: 'var(--brand-orange)', boxShadow: '0 0 10px var(--brand-orange)' }} />
        )}
        {Icon && !isCritical && (
          <div
      ...
      <div className="flex items-end justify-between">
        ...
        <div
          ...
          style={{
            ...
            color: trendType === 'positive' ? 'var(--color-accent-green)' : trendType === 'negative' ? 'var(--color-accent-red)' : 'var(--text-secondary)',
          }}
        >
