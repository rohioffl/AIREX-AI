...
export default function CustomSelect({ value, onChange, options = [], placeholder = 'Select…', className = '', style = {} }) {
  ...
  return (
    <div ref={wrapperRef} className={`relative ${className}`} style={{ display: 'inline-block' }}>
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
        style={triggerStyle}
      ...
      {open && (
        <div style={dropdownStyle}>
          ...
          />
          {normalised.length > 0 && (
            <div style={{ height: 1, background: isDark ? 'rgba(255,255,255,0.06)' : '#F3F4F6', margin: '3px 4px' }} />
          )}
          {normalised.map(opt => (
            <DropdownItem
              key={opt.value}
...
function DropdownItem({ label, isSelected, isDark, onClick, isMuted }) {
