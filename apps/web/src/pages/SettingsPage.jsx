...
export default function SettingsPage() {
  ...
  const canManageIntegrations = canAccessRoute(auth, 'tenant_admin')
  const activeTab = searchParams.get('tab') === 'integrations' && canManageIntegrations
    ? 'integrations'
    : 'general'
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {canManageIntegrations && (
        <div className="flex gap-1 p-1 rounded-lg" style={{ background: 'var(--bg-input)', border: '1px solid var(--border)', width: 'fit-content' }}>
          {[
            ...
            <button
              key={id}
              onClick={() => setSearchParams(id === 'general' ? {} : { tab: id })}
              ...
              style={{
                ...
                color: activeTab === id ? 'var(--text-heading)' : 'var(--text-muted)',
      ...
      {activeTab === 'integrations' && <IntegrationsAdminPage embedded />}
